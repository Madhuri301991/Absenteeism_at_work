...
# create the special class that we are going to use from here to predict new data
class absenteeism_model():
    ...
    def predicated_probability(self):
        if (self.data is not None):
            pred=self.reg.predict_proba(self.data)[:,1]
            return pred
    
    #a function which outputs 0 or 1 based on our model
    def predicated_output_category(self):
        if (self.data is not None):
            pred_outputs=self.reg.predic(self.data)
            return pred_outputs
        
    #predict the outputs and the probabilities and add columns with these values at the end of new data
    def predicated_outputs(self):
        if(self.data is not None):
